...
const TrezorConnect = new require('./trezor-connect.js');
...
class TrezorKeyring extends EventEmitter {
  ...
  signTransaction(address, tx) {
    ...
    return new Promise((resolve, reject) => {
      this.unlock()
          .then(status => {
            setTimeout(_ => {
              TrezorConnect.irchainSignTransaction(
                  ...
                  response => {
                    if (response.success) {
                      ...
                      const addressSignedWith = ircUtil.toChecksumAddress(`0x${signedTx.from.toString('hex')}`);
                      const correctAddress = ircUtil.toChecksumAddress(address);
      ...
      this.unlock()
          .then(status => {
            setTimeout(_ => {
              ...
              TrezorConnect.irchainSignMessage(this._pathFromAddress(withAccount), humanReadableMsg, response => {
                if (response.success) {
  ...
  _normalize(buf) {
    return this._padLeftEven(ircUtil.bufferToHex(buf).substring(2).toLowerCase());
  }
